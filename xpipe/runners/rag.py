...
from xpipe.pricing import calculate_cost
...
# Import additional retrievers from retrievers module
try:
    ...
    RETRIEVERS = {
        "simple_overlap": _retrieve_simple,
        "jaccard": _retrieve_jaccard,
        "bm25": bm25_retrieve,
        "bert": bert_retrieve,
    ...
    # Fallback to basic retrievers if retrievers module not available
...
def _hf_gen(model_id: str, prompt: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if mdl is None or tok is None:
        ...
        # Auto-detect device: prefer GPU, fallback to CPU
...
def build_runner(stage_cfg: Dict[str, Any], models: Dict[str, Any]):
    ...
    def _run(prompt: str, **kw):
        p = {**base, **overrides, **kw}
        # Use unified backend dispatcher for all model types
        if backend == "hf":
            return _hf_gen(model_id, prompt, p)
        elif backend == "ollama":
            return _ollama_gen(model_id, prompt, p)
        elif backend == "anthropic":
            # Use unified call_llm for Anthropic backend
            from xpipe.llm_backends import call_llm
            return call_llm(backend, model_id, prompt, **p)
        else:
...
def run(cfg: Dict[str, Any], models: Dict[str, Any], metrics: MetricLog,
    ...
    for q in queries:
        ...
        synth_model_id = getattr(synth, "_model_id", "")
        judge_model_id = getattr(judge_runner, "_model_id", "") if judge_runner else ""

        retrieval_cost = 0.0  # Retrieval is always free (no LLM involved)
        synth_cost = calculate_cost(synth_model_id, synth_pt, synth_ct)
        judge_cost = calculate_cost(judge_model_id, judge_pt, judge_ct) if judge_runner else 0.0
        total_cost = retrieval_cost + synth_cost + judge_cost
