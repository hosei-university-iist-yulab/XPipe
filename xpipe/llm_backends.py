...
def hf_generate(model_id: str, prompt: str, **kw) -> Dict[str, Any]:
    ...
    **kw : dict
    ...
    if entry is None:
        ...
        mdl.eval()
        _HF_CACHE[model_id] = entry = {"tok": tok, "mdl": mdl, "device": device}
    ...
    elif str(mdl.device).startswith("cuda"):
...
def anthropic_generate(model_id: str, prompt: str, **kw) -> Dict[str, Any]:
    """
    ...
    ----------
    model_id : str
        The Claude model identifier (e.g. "claude-3-haiku-20240307",
    ...
    **kw : dict
    ...
    - Pricing varies by model (see xpipe.pricing for details).
    """
    try:
        import anthropic
    except ImportError:
    ...
    return {"text": text, "usage": usage, "latency_s": dt}
...
BACKENDS = {
    ...
    "anthropic": anthropic_generate  # Anthropic Claude (cloud API)
}
